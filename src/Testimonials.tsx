...
const Testimonials = () => {
  ...
  return (
    <section className="relative py-16 md:py-20 bg-brand-page overflow-hidden">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(900px_500px_at_85%_-10%,rgba(212,83,126,0.12),transparent_60%)]" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(700px_460px_at_-5%_110%,rgba(242,196,206,0.2),transparent_60%)]" />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-brand-shrimp/35 to-transparent" />
      <div className="relative max-w-[1840px] mx-auto px-4 md:px-10">
        <div className="mb-8 flex items-baseline gap-3">
          <span className="h-1.5 w-1.5 rounded-full bg-brand-shrimp" />
      ...
      <div className="relative mx-4 md:mx-6">
        <div className="pointer-events-none absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-brand-page to-transparent z-10" />
        <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-brand-page to-transparent z-10" />
        <div className="flex gap-5 overflow-x-auto pb-4 snap-x snap-mandatory no-scrollbar">
          {reviews.map((rev, i) => (
            <motion.div
              key={i}
              whileHover={{ y: -4 }}
              className="min-h-[360px] min-w-[320px] max-w-[360px] snap-start flex-shrink-0 rounded-md rounded-tr-xl border border-brand-ink/10 bg-brand-card p-6 shadow-[0_18px_50px_rgba(26,26,26,0.07)] flex flex-col"
            >
              <div className="mb-5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-[11px] font-semibold tracking-wider rounded-sm border border-brand-shrimp/40 bg-brand-shrimp/10 px-2.5 py-1 text-brand-shrimp">{rev.status}</div>
                </div>
                <div className="h-8 min-w-[96px] rounded-md border border-brand-ink/10 bg-brand-page/70 px-3 flex items-center justify-center text-sm font-semibold text-brand-ink/75">
                  {rev.company}
                </div>
              </div>
